...
你要将一个分支A的提交历史合并到另一个分支B时，merge 会保留两条分支的提交历史，然后结合两个分支的共同祖先以及两者全部的修改生成一次新的提交，这个新的提交包含了两个分支共同祖先之后的所有提交。同时保留两个分支的提交历史更像是两条路最后交汇在一点。对比的变基，则是将 A 分支的所有提交顺延在 B 分支之后，把 A 分支的所有提交当做 B 分支之后新的修改和提交，更像是将 一条路拼接到另一条路上。

要注意的是两种方法整合后最终所指向的快照始终是一样的，不一样的只是提交历史。

变基的主要作用就是简化提交历史，使提交历史出现在一条线上。
...
**不要对在你的仓库之外有副本的分支进行变基。**


### 变基 VS 合并

总的原则是，只对尚未推送或者分享给别人的本地修改执行变基操作清理历史，从不对已推送至别处的提交执行变基操作，这样你才能享受两种方式带来的便利。


**总结：**

**学完分支的基础知识，应该能够理解分支的实现原理以及由此而来的使用方式包括创建分支、切换分支、合并分支、删除分支、推送分支、拉取分支。使用变基优化提交历史。**
